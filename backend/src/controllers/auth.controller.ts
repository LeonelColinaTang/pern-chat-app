...
export const signup = async (req: Request, res: Response) => {
    ...
    try{
        ...
        if (!fullName || !username || !password || !confirmPassword || !gender){
            ...
            console.log("I GOT HIT IN THE CONTROLLER");
        ...
        if (username.length < 8) {
          return res.status(400).json({error: "Username must have at least 8 characters"});
        }

        if (fullName.split(" ").length < 2) {
          return res
            .status(400)
            .json( {error: "Please include your first and last name"});
        }
